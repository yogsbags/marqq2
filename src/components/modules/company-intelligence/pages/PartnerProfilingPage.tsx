  ...
  if (!artifact || !data) {
    return (
      <Card>
        ...
        <CardContent className="text-sm text-gray-700">Generate to see partner types, value exchange, and activation playbooks.</CardContent>
      </Card>
    )
  }
          ...
          {partnerTypes.length ? (
            partnerTypes.map((p, idx) => (
              <div key={idx} className="border rounded-md p-3">
                ...
                <div className="text-sm text-gray-800 mt-1">Value exchange: {String(p.valueExchange || '—')}</div>
