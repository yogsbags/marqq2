        ...
        <CardContent className="space-y-3">
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-3">
            ...
            <div className="border rounded-md p-3">
              ...
              <div className="text-xs text-gray-600 mt-2">Geo focus</div>
              <div className="text-sm text-gray-900">{geoFocus.length ? geoFocus.join(', ') : '—'}</div>
            </div>
            <div className="border rounded-md p-3">
              <div className="text-xs text-gray-600">Brand voice</div>
              <div className="text-sm text-gray-900">{tone || '—'}</div>
              <div className="text-xs text-gray-600 mt-2">Style</div>
              <div className="text-sm text-gray-900">{style || '—'}</div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-3">
            <div className="border rounded-md p-3">
              <div className="text-xs text-gray-600">Offerings</div>
            ...
            <div className="border rounded-md p-3">
              <div className="text-xs text-gray-600">Primary audience</div>
              <div className="flex flex-wrap gap-2 mt-2">
              ...
              productsServices.slice(0, 12).map((ps, idx) => (
                <div key={idx} className="border rounded-md p-3">
                  <div className="flex items-center justify-between gap-2">
                    ...
                    <div className="text-xs text-gray-600">{safeString(ps.category) || '—'}</div>
                  </div>
                  <div className="text-sm text-gray-900 mt-1">{safeString(ps.description)}</div>
                  <div className="text-xs text-gray-600 mt-2">For: {safeString(ps.targetCustomer) || '—'}</div>
                  <div className="text-xs text-gray-600">Differentiator: {safeString(ps.differentiator) || '—'}</div>
                </div>
              ))
            ) : (
              <div className="text-sm text-gray-700">No products/services detected yet. Re-ingest with a valid website URL.</div>
            )}
          </CardContent>
        </Card>
