...
import { FolderOpen, Check, XCircle, File } from 'lucide-react'
...
export default function FileUpload({
  ...
  return (
    <div className="mb-6">
      ...
      <div
        className={`relative border-2 border-dashed rounded-lg p-6 text-center transition-all ${
          dragActive
            ...
            : 'border-slate-300 bg-slate-50 dark:border-slate-700 dark:bg-slate-950/80'
        ...
        <div className="space-y-2">
          ...
          {uploadedFiles.length > 0 && (
            <p className="text-sm font-semibold text-green-600 flex items-center justify-center gap-1">
              <Check className="w-4 h-4" /> {uploadedFiles.length} file{uploadedFiles.length > 1 ? 's' : ''} uploaded
            </p>
          )}
        </div>
      </div>
      ...
      {/* Error Message */}
      {error && (
        <div className="mt-3 p-3 bg-red-50 dark:bg-red-950/20 border border-red-200 dark:border-red-900/60 rounded-lg">
      ...
      {uploadedFiles.length > 0 && (
        <div className="mt-4 space-y-3">
          {uploadedFiles.map((file, index) => (
            <div
              key={index}
              className="flex items-center gap-3 p-3 bg-white dark:bg-slate-950 border-2 border-slate-200 dark:border-slate-800 rounded-lg hover:border-slate-300 dark:hover:border-slate-700 transition-all"
            >
              {/* Preview */}
              <div className="flex-shrink-0">
                ...
                {fileType === 'pdf' && (
                  <div className="w-16 h-16 bg-red-100 dark:bg-red-950/20 rounded flex items-center justify-center">
              ...
              <button
                ...
                disabled={disabled}
                className="flex-shrink-0 p-2 text-red-500 hover:bg-red-50 dark:hover:bg-red-950/20 rounded-lg transition-colors disabled:opacity-50"
              >
