    ...
    case 'marketing_ideas':
      return {
        ...
        systemPrompt: [
          ...
          ...sharedRules,
          'LOCKED GTM STRATEGY CONTEXT (required when present in inputs.notes / inputs.gtmStrategyContext / inputs.goalAlignment):',
          'Ideas MUST advance the quantified North Star Metric and its metric_definition already locked in the GTM strategy.',
          'Respect business_archetype, metric_tree, and guardrails. Never optimize rejects_as_nsm vanity metrics.',
          'Do NOT redefine ICP, positioning, channel mix, pricing, or a competing GTM plan — those live in the strategy document.',
          'Each idea must state which strategy section sub-goal it advances and a measurable contribution toward the north-star number.',
          'Process (from marketing-ideas SKILL.md):',
          '1) Read product/audience/stage/budget/goal from company profile + LOCKED GTM strategy / goalAlignment (notes/gtmStrategyContext) — never invent a different business or strategy.',
          '2) Select 3-5 MOST relevant ideas from the 139-idea catalog that best close the quantified gap — not generic LLM brainstorms.',
          '3) For each: Idea name, Why it fits (tie to strategy + quantified target), How to start (2-3 steps), Expected outcome (numeric contribution), Resources needed.',
          ...
          'Output JSON shape: { "scores": { "fitScore": number, "actionability": number, "channelDiversity": number }, "summary": string, "stageFit": "pre-launch"|"early"|"growth"|"scale", "budgetBand": "free"|"low"|"medium"|"high", "northStar": { "quantified_target": string, "timeline_target": string }, "ideas": [{ "ideaNumber": number, "name": string, "category": string, "priority": "high"|"medium"|"low", "whyItFits": string, "strategySectionId": string, "contributionToGoal": string, "hooks": string[], "angles": string[], "howToStart": string[], "expectedOutcome": string, "resources": string, "outcomeModule": "paid-ads"|"social-media"|"email-sequence"|"lead-outreach"|"lead-magnets"|"content-strategy"|"partners"|"ab-test"|"user-engagement"|"programmatic-seo"|"referral" }], "hooksToTest": [{ "hook": string, "why": string }], "anglesToTest": [{ "angle": string, "framework": "PAS"|"BAB"|"social_proof"|"feature_benefit"|"direct_response", "hypothesis": string }] }.',
          ...
          'Return exactly 3 to 5 ideas (skill default), not a laundry list. Diversify categories only when they fit this company AND the locked strategy.',
          ...
          'Be concrete and grounded in the company profile + GTM strategy; no generic chatbot/AI filler.',
        ].join(' '),
      };
    case 'icps':
      return {
