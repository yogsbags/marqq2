...
import { normalizeGoalSystem, goalSystemToQuantifiedLabel } from "./gtmNorthStar.js";
 ...
 */
export async function diagnoseBottleneck(groq, { goalSystem, controlLoop, notes }) {
  const g = normalizeGoalSystem(goalSystem);
  ...
  try {
    const completion = await groq.chat.completions.create({
      ...
      messages: [
        {
          role: "system",
          content: `${CONTROL_LOOP_PRINCIPLES}
 ...
 */
export async function proposeInterventions(groq, { goalSystem, controlLoop, diagnosis }) {
  const g = normalizeGoalSystem(goalSystem);
  const loop = normalizeControlLoopState(controlLoop || {}, g);
