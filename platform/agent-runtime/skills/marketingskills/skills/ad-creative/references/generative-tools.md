...
#### Marqq fallback contract

When Marqq reaches the Fal.ai video fallback, use `bytedance/seedance-2.0/fast/image-to-video` by default. The automation supplies a prompt plus a first and last reference image; if either image is missing, generate it with Fal's `fal-ai/nano-banana-pro` (or edit the first frame with `fal-ai/nano-banana-pro/edit` for a more coherent closing frame). Keep the subject, product identity, camera direction, lighting, and aspect ratio consistent between the two frame prompts. Use `generate_audio: true` unless the user explicitly requests silent output. Use text-to-video only when reference-frame generation is disabled.
